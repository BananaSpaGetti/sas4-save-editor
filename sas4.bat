@echo off
setlocal
rem Start the whole save editor with a double click: all three panels as tabs of one
rem window, so granting a gun and then setting a level is not two windows and two pickers.
rem
...
rem
rem This is the only thing in this folder meant to be double-clicked. The code, and the
rem launchers for the single panels -- sas4-quick.bat, sas4-gear.bat, sas4-gui.bat -- are in
rem tools\, out of the way of someone who just wants to edit a save.
rem
rem Pass a save file to edit something other than the live profile:
rem     sas4.bat saves\profile-000.save

cd /d "%~dp0"

if not exist "tools\sas4_hub.py" (
    echo Cannot find tools\sas4_hub.py next to this file.
    echo.
    ...
    echo run sas4.bat from the extracted copy.
    echo.
    pause
    exit /b 1
)
