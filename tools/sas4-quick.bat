@echo off
setlocal
rem Start the quick editor with a double click -- level, money, and granting an item.
rem
...
rem     sas4-quick.bat saves\profile-000.save

cd /d "%~dp0"

if not exist "sas4_quick.py" (
    echo Cannot find sas4_quick.py next to this file.
    echo.
    ...
    echo run sas4-quick.bat from the extracted copy.
    echo.
    pause
    exit /b 1
)
