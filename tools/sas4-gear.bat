@echo off
setlocal
rem Start the armoury with a double click -- grant weapons and equipment.
rem
...
rem     sas4-gear.bat saves\profile-000.save

cd /d "%~dp0"

if not exist "sas4_gear.py" (
    echo Cannot find sas4_gear.py next to this file.
    echo.
    ...
    echo run sas4-gear.bat from the extracted copy.
    echo.
    pause
    exit /b 1
)
