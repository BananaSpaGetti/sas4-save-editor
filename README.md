...
There are two ways to run the command line, and they need different things.

- **Python 3** (`py` on Windows), from <https://www.python.org/downloads/> -- tick **Add
  python.exe to PATH** on the installer's first screen. This is what the editor window
  (`sas4.bat`) needs, and what `py tools/sas4.py` needs. Nothing here needs a third-party
...
- **Or no Python at all**, for the command line only: `c/` holds a C port of it that builds
  to a single `sas4.exe` with the same commands, the same output and no runtime
  dependencies. Building it needs a C compiler once (see [Building the C command
  line](#building-the-c-command-line)); running it afterwards needs nothing. **The editor
  window still requires Python** -- the port covers the command line, not the GUI.
...
|---|---|
| `No working Python was found` | Install Python 3 and tick *Add python.exe to PATH*, or build `c/sas4.exe` and use `tools\sas4-cli.bat`, which needs no Python |
...
tests/            `py tests/test_sas4.py` runs them, standard library only
```
...
## Building the C command line

`c/` is a port of the command line to C. It exists so the command line can run on a machine
with no Python on it; it is checked against the Python rather than trusted, and the two are
expected to agree byte for byte.

The zip ships the sources, not a binary. Build it once:
...
Any C11 compiler works. On Windows, [w64devkit](https://github.com/skeeto/w64devkit) is a
single unzip with no installer; MSYS2 works too. Then run it through its launcher, which
takes the same arguments the Python one does:

```
tools\sas4-cli.bat view
tools\sas4-cli.bat --file saves\profile-000.save give 129 --kind weapon
```

or call `c\sas4.exe` directly. Every command `py tools/sas4.py` accepts, it accepts.

**The editor window is not ported.** `sas4.bat` still starts the Python one, and still needs
Python and tkinter.

To check the port yourself:

```
cd c
make test                            the C unit tests
cd ..
py c/tests/differential_core.py      the format and edit machinery, against the Python
py c/tests/differential_cli.py       all seventeen subcommands, against the Python
```

The differential harnesses run both implementations over the same inputs and compare the
results. They print counts only -- never a value from your profile, and never a save's path,
which carries your Steam account number.
...
Standard library only, so they run wherever the Python tools do. Every save they touch is
generated by `sas4_model.py` in a temporary directory -- they never read a real profile and
never write outside that directory.

The C port has its own tests, and two differential harnesses that check it against this
Python rather than against a fixture of their own -- see [Building the C command
line](#building-the-c-command-line).
